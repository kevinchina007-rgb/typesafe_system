  ...
  protected final def reviewRoutes: HttpRoutes[F] = HttpRoutes.of[F] {
    case request @ GET -> Root / "api" / "reviews" / "mine" =>
      for
        ...
        reviews <- reviewApplicationService.listMyReviews(currentUserId)
    ...
    case request @ GET -> Root / "api" / "reviews" =>
      for
        ...
        reviews <- reviewApplicationService.listReviewsByResource(currentUserId, resourceType, resourceIdValue.trim)
        response <- Ok(ReviewListResponseDto(reviews.map(ReviewResponseDto.fromView)).asJson)
      yield response
    ...
    case request @ GET -> Root / "api" / "reviews" / "summary" =>
      for
        ...
        resourceIdValue <- fromEither(request.params.get("resourceId").filter(_.trim.nonEmpty).toRight(SharedValidationError.RequiredFieldWasEmpty("resourceId")))
        resourceType <- fromEither(parseReviewResourceType(resourceTypeValue))
        summary <- reviewApplicationService.getResourceSummary(currentUserId, resourceType, resourceIdValue.trim)
    ...
    case request @ GET -> Root / "api" / "reviews" / "eligibility" =>
      for
        ...
        eligibility <- reviewApplicationService.checkEligibility(currentUserId, OrderItemId(orderItemIdValue))
    ...
    case request @ POST -> Root / "api" / "reviews" / "images" =>
      for
        ...
        now <- currentInstantF
        imageRef <- reviewApplicationService.uploadImage(currentUserId, imageFileName, imageContentType, imageBytes, now)
    ...
    case request @ DELETE -> Root / "api" / "reviews" / reviewIdValue =>
      for
        ...
        now <- currentInstantF
        _ <- reviewApplicationService.deleteReview(ReviewId(reviewIdValue), currentUserId, now)
        response <- NoContent()
      yield response
  }
