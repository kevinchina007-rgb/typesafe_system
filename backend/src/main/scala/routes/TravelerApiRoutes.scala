...
trait TravelerApiRoutes[F[_]: Async] extends Http4sDsl[F]:
  ...
  protected final def travelerRoutes: HttpRoutes[F] = HttpRoutes.of[F] {
    ...
    case request @ GET -> Root / "api" / "users" / userIdValue / "travelers" =>
      for
        currentUserId <- requireCurrentUserId(request)
        _ <- if currentUserId == UserId(userIdValue) then Async[F].unit else Async[F].raiseError(com.typesafe.travel.auth.domain.AuthError.SessionActorDidNotMatch)
        travelers <- travelerProfileRepository.findTravelerProfilesByOwnerUserId(UserId(userIdValue))
        response <- Ok(TravelerListResponseDto(travelers.map(TravelerResponseDto.fromDomain)).asJson)
      yield response

    case request @ DELETE -> Root / "api" / "users" / userIdValue / "travelers" / travelerIdValue =>
      for
        currentUserId <- requireCurrentUserId(request)
        _ <- if currentUserId == UserId(userIdValue) then Async[F].unit else Async[F].raiseError(com.typesafe.travel.auth.domain.AuthError.SessionActorDidNotMatch)
        _ <- travelerProfileService.deleteTravelerProfile(UserId(userIdValue), TravelerId(travelerIdValue))
        response <- NoContent()
      yield response
