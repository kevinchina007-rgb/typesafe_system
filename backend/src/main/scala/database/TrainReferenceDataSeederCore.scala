// TrainReferenceDataSeederCore handles railway manager and train seeding.
...
import TrainReferenceDataSeederRoutes.timestamptzLiteral

import java.time.{Instant, LocalDate, LocalTime, ZoneOffset}

object TrainReferenceDataSeederCore:
  val DemoManagerId = "manager-train-hh306"
  val DemoManagerEmail = "ops@hh306.example"
  val DemoManagerDisplayName = "哼哼306"
  val DemoManagerOperatorCode = "HH306"
  val DemoManagerPassword = "Train2026!"
  ...
  val stationNameByCode: Map[String, String] = Map(
    "BJS" -> "\u5317\u4eac\u5357",
    "BJX" -> "\u5317\u4eac\u897f",
    "TJS" -> "\u5929\u6d25\u5357",
    "JNW" -> "\u6d4e\u5357\u897f",
  ...
  val baseDemoTrains = List(
    TrainSeed(
      ...
      stops = List(
        TrainStopSeed("BJS", stationNameByCode("BJS"), None, Some("2026-06-01T02:00:00Z")),
        TrainStopSeed("TJS", stationNameByCode("TJS"), Some("2026-06-01T02:28:00Z"), Some("2026-06-01T02:31:00Z")),
        TrainStopSeed("JNW", stationNameByCode("JNW"), Some("2026-06-01T04:10:00Z"), Some("2026-06-01T04:13:00Z")),
        TrainStopSeed("NJS", stationNameByCode("NJS"), Some("2026-06-01T07:10:00Z"), Some("2026-06-01T07:13:00Z")),
        TrainStopSeed("SHH", stationNameByCode("SHH"), Some("2026-06-01T09:05:00Z"), None)
      ),
    ...
    TrainSeed(
      ...
      saleStartsAt = "2026-05-18T00:00:00Z",
      stops = List(
        TrainStopSeed("SHH", stationNameByCode("SHH"), None, Some("2026-06-01T01:00:00Z")),
        TrainStopSeed("SZB", stationNameByCode("SZB"), Some("2026-06-01T01:23:00Z"), Some("2026-06-01T01:26:00Z")),
        TrainStopSeed("HZD", stationNameByCode("HZD"), Some("2026-06-01T02:05:00Z"), Some("2026-06-01T02:08:00Z")),
        TrainStopSeed("NGB", stationNameByCode("NGB"), Some("2026-06-01T03:40:00Z"), Some("2026-06-01T03:43:00Z")),
        TrainStopSeed("WZS", stationNameByCode("WZS"), Some("2026-06-01T05:30:00Z"), None)
      ),
      seatInventories = defaultSeatInventories,
      segmentPrices = List(
    ...
    TrainSeed(
      ...
      saleStartsAt = "2026-05-18T00:00:00Z",
      stops = List(
        TrainStopSeed("GZQ", stationNameByCode("GZQ"), None, Some("2026-06-01T02:00:00Z")),
        TrainStopSeed("SZN", stationNameByCode("SZN"), Some("2026-06-01T02:34:00Z"), Some("2026-06-01T02:37:00Z")),
        TrainStopSeed("XMN", stationNameByCode("XMN"), Some("2026-06-01T05:10:00Z"), None)
      ),
      seatInventories = defaultSeatInventories,
      segmentPrices = List(
    ...
    TrainSeed(
      ...
      saleStartsAt = "2026-05-18T00:00:00Z",
      stops = List(
        TrainStopSeed("CDD", stationNameByCode("CDD"), None, Some("2026-06-01T00:20:00Z")),
        TrainStopSeed("CQB", stationNameByCode("CQB"), Some("2026-06-01T01:34:00Z"), Some("2026-06-01T01:37:00Z")),
        TrainStopSeed("WUH", stationNameByCode("WUH"), Some("2026-06-01T04:20:00Z"), Some("2026-06-01T04:23:00Z")),
        TrainStopSeed("CSN", stationNameByCode("CSN"), Some("2026-06-01T06:25:00Z"), None)
      ),
      seatInventories = defaultSeatInventories,
      segmentPrices = List(
    ...
    TrainSeed(
      ...
      saleStartsAt = "2026-05-18T00:00:00Z",
      stops = List(
        TrainStopSeed("BJS", stationNameByCode("BJS"), None, Some("2026-06-01T03:00:00Z")),
        TrainStopSeed("ZZD", stationNameByCode("ZZD"), Some("2026-06-01T05:35:00Z"), Some("2026-06-01T05:38:00Z")),
        TrainStopSeed("WUH", stationNameByCode("WUH"), Some("2026-06-01T07:55:00Z"), Some("2026-06-01T07:58:00Z")),
        TrainStopSeed("GZQ", stationNameByCode("GZQ"), Some("2026-06-01T12:10:00Z"), None)
      ),
      seatInventories = defaultSeatInventories,
      segmentPrices = List(
  ...
  lazy val generatedTrainSeeds = TrainReferenceDataSeederRoutes.generatedTrainSeeds
  lazy val demoTrains = baseDemoTrains ++ generatedTrainSeeds
  def seedIfNeeded(transactor: Transactor[IO]): IO[Unit] =
    for
      _ <- repairExistingTrainMetadata(transactor)
  ...
  def seedManagerIfNeeded(transactor: Transactor[IO]): IO[Unit] =
    for
      _ <- insertManager(transactor)
    yield ()

  def repairExistingTrainMetadata(transactor: Transactor[IO]): IO[Unit] =
    for
      _ <- repairExistingTrainStopNames(transactor)
      _ <- repairExistingTrainSeatLabels(transactor)
    yield ()

  def repairExistingTrainStopNames(transactor: Transactor[IO]): IO[Unit] =
    val stationNameCases =
      Fragment.const(
        stationNameByCode.toList
          .sortBy(_._1)
          .map { case (stationCode, stationName) => s"when '$stationCode' then '$stationName'" }
          .mkString("\n        ")
      )
    val stationCodes =
      Fragment.const(
        stationNameByCode.keys.toList.sorted.map(stationCode => s"'$stationCode'").mkString(", ")
      )
    sql"""
      update train_stops
      ...
      where station_code in ($stationCodes)
    """.update.run.transact(transactor).void

  def repairExistingTrainSeatLabels(transactor: Transactor[IO]): IO[Unit] =
    sql"""
      update train_seats
  ...
  def insertManager(transactor: Transactor[IO]): IO[Unit] =
    for
  ...
  def insertManagerRows(passwordHash: String): ConnectionIO[Unit] =
  ...
  def seedTrainIfMissing(transactor: Transactor[IO], seed: TrainSeed): IO[Unit] =
    for
  ...
  def insertTrain(transactor: Transactor[IO], seed: TrainSeed): IO[Unit] =
    insertTrainRows(seed).transact(transactor).void

  def insertTrainRows(seed: TrainSeed): ConnectionIO[Unit] =
  ...
  def insertStops(seed: TrainSeed): ConnectionIO[Unit] =
  ...
  def insertSeatInventories(seed: TrainSeed): ConnectionIO[Unit] =
  ...
  def insertSeats(seed: TrainSeed): ConnectionIO[Unit] =
    seed.seatInventories.traverse_ { inventory =>
  ...
  def insertSegmentPrices(seed: TrainSeed, stopIds: Map[String, String]): ConnectionIO[Unit] =
  ...
  def insertRefundPolicies(seed: TrainSeed): ConnectionIO[Unit] =
  ...
  def generateSeatRows(trainId: String, inventory: TrainSeatInventorySeed): Vector[SeatRowInsert] =
  ...
  def parseSeatLayoutSpec(layoutSpec: String): Vector[(String, String)] =
    layoutSpec
      .split(',')
      .toVector
      .map(_.trim)
  ...
  def inventoryId(trainId: String, seatClass: String): String =
    s"$trainId-${seatClass.toLowerCase.replace('_', '-')}"

  def stopId(trainId: String, sequenceNo: Int): String =
    s"$trainId-stop-$sequenceNo"

  def segmentPriceId(trainId: String, price: TrainSegmentPriceSeed): String =
    s"$trainId-${price.seatClass.toLowerCase}-${price.fromStationCode.toLowerCase}-${price.toStationCode.toLowerCase}"

  def refundPolicyId(trainId: String, policy: TrainRefundPolicySeed): String =
    s"$trainId-${policy.startOffsetMinutesBeforeDeparture}-${policy.endOffsetMinutesBeforeDeparture}"

  def formatTrainSeatLabel(carriageNo: Int, seatNo: String): String =
