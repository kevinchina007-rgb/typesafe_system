// ReferenceDataSeederHotels handles hotel and hotel-manager reference data.
...
object ReferenceDataSeederHotels:
  def seedHotels(transactor: Transactor[IO]): IO[Unit] =
    seedHotelDemoData(transactor)

  def seedHotelManagers(transactor: Transactor[IO]): IO[Unit] =
    List(
      sql"""
  ...
  val hotelDemoPassword = "Hotel2026!"
  val hotelDemoCreatedAt = Instant.parse("2026-05-18T00:00:00Z")
  val hotelInventoryStartDate = LocalDate.parse("2026-06-01")
  val hotelInventoryEndDate = LocalDate.parse("2026-07-31")
  val hotelInventoryDates =
    Iterator.iterate(hotelInventoryStartDate)(_.plusDays(1)).takeWhile(!_.isAfter(hotelInventoryEndDate)).toVector
  val hotelRoomTypeTemplates = List(
  ...
  def hotelSeed(hotelId: String, hotelName: String, managerId: String, managerEmail: String, managerDisplayName: String): HotelSeed =
    HotelSeed(hotelId, hotelName, managerId, managerEmail, managerDisplayName)
  val hotelCitySeeds = List(
    HotelCitySeed(
      "北京",
      List(
  ...
  def seedHotelDemoData(transactor: Transactor[IO]): IO[Unit] =
  ...
  final case class HotelCitySeed(cityName: String, hotels: List[HotelSeed])
  final case class HotelSeed(
      hotelId: String,
      hotelName: String,
      managerId: String,
      managerEmail: String,
  ...
  def insertOrUpdateHotel(hotelId: String, name: String, location: String, status: String, createdAt: Instant): ConnectionIO[Int] =
  ...
  def insertOrUpdateHotelManager(
      managerId: String,
      hotelId: String,
      email: String,
      displayName: String,
  ...
  def insertOrUpdateHotelRoomType(
      roomTypeId: String,
      hotelId: String,
      roomTypeName: String,
      capacity: Int,
  ...
  def insertOrUpdateHotelRoomInventory(
      inventoryId: String,
      roomTypeId: String,
