...
import com.typesafe.travel.persistence.attraction.AttractionPlannerReadSql
import com.typesafe.travel.shared.kernel.{AttractionId, AttractionTicketSessionId, ManagerId, TicketTypeId}
  ...
  def listManaged(connection: Connection, input: ListManagedAttractionsPlannerRequest): IO[AttractionListPlannerResponse] =
    IO.blocking {
      PlainSqlSupport.withStatement(
        ...
        AttractionListPlannerResponse(PlainSqlSupport.queryList(statement)(AttractionPlannerReadSql.readAttraction(connection)))
      }
    }
