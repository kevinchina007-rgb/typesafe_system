...
def hashPassword(rawPassword: String): IO[String] =
  IO.delay {
...
def hashPasswordForLoginEmail(rawPassword: String, loginEmail: EmailAddress): IO[String] =
  IO.delay {
    val normalizedPassword = rawPassword.trim
...
def verifyPassword(rawPassword: String, passwordHash: String): IO[Boolean] =
  IO.delay {
