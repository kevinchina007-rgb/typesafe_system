...
object RecallTourGroupMessagePlanner extends ConnectionApiPlan[RecallTourGroupMessagePlannerRequest, TourGroupMessageListResponse]:

  override val name: String = "RecallTourGroupMessagePlanner"

  override def plan(input: RecallTourGroupMessagePlannerRequest, connection: Connection): IO[TourGroupMessageListResponse] =
    for
