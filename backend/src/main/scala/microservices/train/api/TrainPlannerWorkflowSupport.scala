// TrainPlannerWorkflowSupport 提供火车规划流程中的站点匹配、数据库读取、席位分配和订单校验辅助函数。
package com.typesafe.travel.train.api
...
object TrainPlannerWorkflowSupport:
    ...
    queryTrains(
      ...
      if filteredResponse.nonEmpty then IO.pure(TrainListPlannerResponse(filteredResponse))
        ...
        ensureReferenceData(connection) *>
          queryTrains(connection, baseSql, List(input.date.orNull, input.date.orNull), now).map(response => TrainListPlannerResponse(response.filter(train => trainMatchesSearch(train, input))))
    }
  ...
  def resolveTrainRoutePricing(connection: Connection, trainId: String, fromStopId: String, toStopId: String, seatClass: String): Option[TrainRoutePricing] =
    if List(trainId, fromStopId, toStopId, seatClass).exists(value => value == null || value.trim.isEmpty) then None
      ...
      stopSequenceById.get(fromStopId).flatMap { fromSequenceNo =>
        stopSequenceById.get(toStopId).flatMap { toSequenceNo =>
