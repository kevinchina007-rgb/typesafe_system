...
object OrderPlannerPlainSqlSupport:
  final case class TrainOrderPricing(unitPrice: BigDecimal, totalPrice: BigDecimal, currency: String)

  val orderSelectSql: String =
    """
  ...
  def updateFlightTravelerSelection(connection: Connection, orderId: String, travelerIds: List[String]): Unit =
  ...
  def updateStatus(connection: Connection, orderId: String, status: String, timestampColumn: Option[String], timestamp: Option[Instant]): IO[OrderPlannerResponse] =
    IO.blocking {
  ...
  def refundStatus(connection: Connection, input: RefundDecisionPlannerRequest, status: String, timestampColumn: Option[String], now: Instant): IO[OrderPlannerResponse] =
    IO.blocking {
  ...
  def findRequired(connection: Connection, orderId: String): OrderPlannerResponse =
  ...
  def isOrderPayable(orderStatus: String): Boolean =
  ...
  def validateOrderCanTransitionToPayment(connection: Connection, orderId: String): IO[Unit] =
    IO.blocking {
  ...
  def releaseTrainSeatAllocations(connection: Connection, orderId: String): IO[Unit] =
    IO.blocking {
  ...
  def expireTrainOrdersIfNeeded(connection: Connection, now: Instant): IO[Unit] =
    IO.blocking {
      ...
      val expiredOrderIds = PlainSqlSupport.withStatement(
        ...
        try
          ...
          while resultSet.next() do ids += resultSet.getString("order_id")
          ids.result().distinct
        finally resultSet.close()
      }
  ...
  def readOrder(connection: Connection, resultSet: ResultSet): OrderPlannerResponse =
  ...
  def readLineItems(connection: Connection, orderId: String): List[OrderLineItemPlannerResponse] =
  ...
  def resolveOrderTotalAmount(connection: Connection, orderId: String, fallbackAmount: BigDecimal): BigDecimal =
  ...
  def refreshOrderTotals(connection: Connection, orderId: String, totalAmount: BigDecimal): Unit =
  ...
  def readPayments(connection: Connection, orderId: String): List[PaymentPlannerResponse] =
  ...
  def readRefunds(connection: Connection, orderId: String): List[RefundPlannerResponse] =
  ...
  def enrichHotelSummaryLabel(resultSet: ResultSet): String =
    val snapshotJson = Option(resultSet.getString("snapshot_json")).getOrElse("{}")
  ...
  def enrichTrainSummaryLabel(resultSet: ResultSet, trainPricing: Option[TrainOrderPricing]): String =
    val snapshotJson = Option(resultSet.getString("snapshot_json")).getOrElse("{}")
    val baseJson = parse(snapshotJson).getOrElse(Json.obj())
    val travelerIdsJson = Option(resultSet.getString("traveler_ids_json")).flatMap(value => parse(value).toOption).getOrElse(Json.arr())
  ...
  def resolveTrainPricing(trainId: String, fromStopId: String, toStopId: String, seatClass: String, travelerCount: Int, connection: Connection): Option[TrainOrderPricing] =
  ...
  def addStringField(jsonObject: io.circe.JsonObject, fieldName: String, fieldValue: String | Null): io.circe.JsonObject =
