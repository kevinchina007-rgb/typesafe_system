...
final class LiveAttractionBookingApplicationService[F[_]: MonadThrow](
  ...
  override def addAttractionItemToOrder(
    ...
    for
      ...
      _ <- ensureTicketTypeInventoryAvailable(ticketType, useDate, travelerProfiles.size)
  ...
  private def ensureTicketTypeInventoryAvailable(ticketType: TicketType, useDate: LocalDate, requestedQuantity: Int): F[Unit] =
