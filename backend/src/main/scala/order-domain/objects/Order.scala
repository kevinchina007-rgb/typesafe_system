...
// Order 仍然是交易主聚合根。
// Flight / Hotel / Train / Attraction 都先把自己的业务快照交给 Order，
// 然后由 Order 统一管理支付、退款、确认和取消等交易状态。
final case class Order private[domain] (
  ...
  // 各 addXxxOrderItem 方法的职责很克制：
  // 这里只验证快照是否自洽，再把它包装成 OrderLineItem，
  // 不在这里做库存锁定或外部查询。
  def addFlightOrderItem(orderItemId: OrderItemId, flightBookingSnapshot: FlightBookingSnapshot): Either[OrderError, Order] =
    for
      _ <- validateFlightBookingSnapshot(flightBookingSnapshot)
      orderWithLineItem <- addOrderLineItem(createReservedFlightOrderItem(orderItemId, flightBookingSnapshot))
    yield orderWithLineItem

  def addHotelOrderItem(orderItemId: OrderItemId, hotelBookingSnapshot: HotelBookingSnapshot): Either[OrderError, Order] =
    for
      _ <- validateHotelBookingSnapshot(hotelBookingSnapshot)
      updatedOrder <- addOrderLineItem(createReservedHotelOrderItem(orderItemId, hotelBookingSnapshot))
    yield updatedOrder

  def addTrainOrderItem(orderItemId: OrderItemId, trainBookingSnapshot: TrainBookingSnapshot): Either[OrderError, Order] =
    for
      _ <- validateTrainBookingSnapshot(trainBookingSnapshot)
      updatedOrder <- addOrderLineItem(createReservedTrainOrderItem(orderItemId, trainBookingSnapshot))
    yield updatedOrder

  def addAttractionOrderItem(orderItemId: OrderItemId, attractionTicketSnapshot: AttractionTicketSnapshot): Either[OrderError, Order] =
    for
      _ <- validateAttractionTicketSnapshot(attractionTicketSnapshot)
      updatedOrder <- addOrderLineItem(createReservedAttractionOrderItem(orderItemId, attractionTicketSnapshot))
    yield updatedOrder
  ...
  def authorizeOrderPayment(paymentId: PaymentId, paymentAmount: Money, paymentMethod: PaymentMethod, authorizedAt: Instant): Either[OrderError, Order] =
  ...
  def captureAuthorizedPayment(paymentId: PaymentId, capturedAt: Instant): Either[OrderError, Order] =
    updatePayment(paymentId) { payment =>
      payment.paymentStatus match
        case PaymentStatus.Authorized => Right(payment.markCapturedPayment(capturedAt))
        case _                        => Left(OrderError.PaymentCouldNotBeCaptured(paymentId, payment.paymentStatus))
    }.map(_.refreshOrderStatusAfterFinancialChange)

  def failAuthorizedPayment(paymentId: PaymentId): Either[OrderError, Order] =
    updatePayment(paymentId) { payment =>
      payment.paymentStatus match
        case PaymentStatus.Authorized => Right(payment.markFailedPayment)
        case _                        => Left(OrderError.PaymentCouldNotBeFailed(paymentId, payment.paymentStatus))
    }

  def requestOrderRefund(refundId: RefundId, refundAmount: Money, refundReason: String, requestedAt: Instant): Either[OrderError, Order] =
    for
      _ <- orderStatus match
  ...
  def approveRequestedRefund(refundId: RefundId, approvedAt: Instant): Either[OrderError, Order] =
    updateRefund(refundId) { refund =>
      refund.refundStatus match
        case RefundStatus.Requested => Right(refund.markApprovedRefund(approvedAt))
        case _                      => Left(OrderError.RefundCouldNotBeApproved(refundId, refund.refundStatus))
    }

  def rejectRequestedRefund(refundId: RefundId): Either[OrderError, Order] =
    updateRefund(refundId) { refund =>
      refund.refundStatus match
        case RefundStatus.Requested => Right(refund.markRejectedRefund)
        case _                      => Left(OrderError.RefundCouldNotBeRejected(refundId, refund.refundStatus))
    }

  def settleApprovedRefund(refundId: RefundId, settledAt: Instant): Either[OrderError, Order] =
    updateRefund(refundId) { refund =>
      refund.refundStatus match
        case RefundStatus.Approved => Right(refund.markSettledRefund(settledAt))
        case _                     => Left(OrderError.RefundCouldNotBeSettled(refundId, refund.refundStatus))
    }.map(_.refreshOrderStatusAfterFinancialChange)
  ...
  def confirmSupplierOrderItem(orderItemId: OrderItemId, managerId: ManagerId, note: Option[String], decidedAt: Instant): Either[OrderError, Order] =
  ...
  def rejectSupplierOrderItem(orderItemId: OrderItemId, managerId: ManagerId, reason: String, decidedAt: Instant): Either[OrderError, Order] =
    updateOrderLineItem(orderItemId) {
  ...
  private def addOrderLineItem(orderLineItem: OrderLineItem): Either[OrderError, Order] =
    orderStatus match
      case OrderStatus.Draft =>
        ensureMatchingCurrency(orderLineItem.bookedMoney).map(_ => copy(orderLineItems = orderLineItems :+ orderLineItem))
      case _ =>
        Left(OrderError.OrderItemsCouldOnlyBeAddedInDraft(orderId, orderStatus))

  private def validateFlightBookingSnapshot(flightBookingSnapshot: FlightBookingSnapshot): Either[OrderError, Unit] =
  ...
  private def validateHotelBookingSnapshot(hotelBookingSnapshot: HotelBookingSnapshot): Either[OrderError, Unit] =
  ...
  private def validateTrainBookingSnapshot(trainBookingSnapshot: TrainBookingSnapshot): Either[OrderError, Unit] =
  ...
  private def validateAttractionTicketSnapshot(attractionTicketSnapshot: AttractionTicketSnapshot): Either[OrderError, Unit] =
  ...
  private def updatePayment(paymentId: PaymentId)(paymentUpdater: Payment => Either[OrderError, Payment]): Either[OrderError, Order] =
    orderPayments.indexWhere(_.paymentId == paymentId) match
      case -1           => Left(OrderError.PaymentWasNotFound(orderId, paymentId))
      case paymentIndex => paymentUpdater(orderPayments(paymentIndex)).map(updatedPayment => copy(orderPayments = orderPayments.updated(paymentIndex, updatedPayment)))

  private def updateRefund(refundId: RefundId)(refundUpdater: Refund => Either[OrderError, Refund]): Either[OrderError, Order] =
    orderRefunds.indexWhere(_.refundId == refundId) match
      case -1          => Left(OrderError.RefundWasNotFound(orderId, refundId))
      case refundIndex => refundUpdater(orderRefunds(refundIndex)).map(updatedRefund => copy(orderRefunds = orderRefunds.updated(refundIndex, updatedRefund)))

  private def updateOrderLineItem(orderItemId: OrderItemId)(orderLineItemUpdater: OrderLineItem => Either[OrderError, OrderLineItem]): Either[OrderError, Order] =
    orderLineItems.indexWhere(_.orderItemId == orderItemId) match
      case -1                => Left(OrderError.OrderItemWasNotFound(orderId, orderItemId))
      case orderLineItemIndex => orderLineItemUpdater(orderLineItems(orderLineItemIndex)).map(updatedOrderLineItem => copy(orderLineItems = orderLineItems.updated(orderLineItemIndex, updatedOrderLineItem)))

  private def refreshOrderStatusAfterFinancialChange: Order =
    // 订单状态由资金状态和退款状态派生，不依赖前端猜测。
  ...
  private def refreshOrderStatusAfterSupplierDecision(decidedAt: Instant): Order =
    if orderStatus == OrderStatus.Confirmed && allSupplierReviewDecisionsConfirmed then copy(confirmedAt = confirmedAt.orElse(Some(decidedAt)))
    else this

enum OrderError(val message: String) extends DomainError:
  // OrderError 是交易根可解释性的核心：所有对外失败最终都可以追到这里。
