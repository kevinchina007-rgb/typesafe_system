...
final class OrderSpec extends FunSuite:
  ...
  test("draft order rejects mixed currencies") {
    val draftOrder =
      newDraftOrder(OrderId("order-1"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
          OrderItemId("item-1"),
  ...
  test("order aggregates item totals across flight and hotel items") {
    val draftOrder =
      newDraftOrder(OrderId("order-2"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
          OrderItemId("item-1"),
          testFlightBookingSnapshot
        )
  ...
  test("capturing enough payment confirms order and order items") {
    val confirmedOrder =
      newDraftOrder(OrderId("order-3"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
          OrderItemId("item-1"),
  ...
  test("supplier confirm accepts optional note after payment confirmation") {
    val supplierConfirmedOrder =
      newDraftOrder(OrderId("order-3b"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
          OrderItemId("item-1"),
          testFlightBookingSnapshot.copy(
            travelerIds = Vector(TravelerId("traveler-1"), TravelerId("traveler-2")),
  ...
  test("supplier reject requires non-empty reason") {
    val rejectAttempt =
      newDraftOrder(OrderId("order-3c"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
          OrderItemId("item-1"),
          testFlightBookingSnapshot.copy(
            travelerIds = Vector(TravelerId("traveler-1"), TravelerId("traveler-2")),
  ...
  test("mixed booking allows independent supplier decisions per item") {
    val decidedOrder =
      newDraftOrder(OrderId("order-3d"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
  ...
  test("refund request cannot exceed remaining refundable balance") {
    val confirmedOrder =
      newDraftOrder(OrderId("order-4"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
          OrderItemId("item-1"),
          testFlightBookingSnapshot
        )
  ...
  test("settled refund moves order to refunded when full amount is returned") {
    val refundedOrder =
      newDraftOrder(OrderId("order-5"), UserId("user-1"), Currency.USD, orderCreatedAtInstant)
        .addFlightOrderItem(
          OrderItemId("item-1"),
          testFlightBookingSnapshot
        )
