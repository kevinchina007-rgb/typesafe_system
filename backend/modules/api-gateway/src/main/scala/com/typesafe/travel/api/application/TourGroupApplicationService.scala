...
final class LiveTourGroupApplicationService[F[_]: MonadThrow](
  ...
  override def createGroup(
    ...
    for
      ...
      group <- TourGroup.create(groupId, organizerUserId, title, description, destination, startDate, endDate, capacity, createdAt).liftTo[F]
      organizerMembership = TourGroupMembership.createOrganizerMembership(membershipId, groupId, organizerUserId, createdAt)
  ...
  override def joinGroup(groupId: TourGroupId, userId: UserId, joinedAt: Instant): F[TourGroupDetailsView] =
    for
      ...
      _ <- details.memberships.find(m => m.userId == userId && m.status == TourGroupMembershipStatus.Active) match
        ...
        case None    => ().pure[F]
      membershipId <- tourGroupRepository.nextMembershipId
      membership = TourGroupMembership.createMemberMembership(membershipId, groupId, userId, joinedAt)
  ...
  override def createPlanItem(
    ...
    for
      ...
      planItem <- GroupPlanItem.create(planItemId, groupId, itemType, title, description, scheduledAt, endsAt, sequenceNo).liftTo[F]
  ...
  override def createPlanOption(
    ...
    for
      ...
      option <- GroupPlanOption.create(optionId, planItemId, resourceType, resourceId, resourceVariantCode, resourceContext, label, description, defaultQuantity).liftTo[F]
  ...
  override def createSelection(
    ...
    for
      ...
      selection <- GroupPlanSelection.create(selectionId, groupId, planItemId, optionId, membership.membershipId, quantity, createdAt).liftTo[F]
