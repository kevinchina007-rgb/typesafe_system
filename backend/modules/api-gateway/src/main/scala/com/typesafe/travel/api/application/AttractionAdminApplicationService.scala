...
final class LiveAttractionAdminApplicationService[F[_]: MonadThrow](
  ...
  override def createAttraction(
    ...
    for
      ...
      attraction <- Attraction.create(attractionId, attractionManager.managerId, attractionName, city, location, description, createdAt).liftTo[F]
  ...
  override def createTicketType(
    ...
    for
      ...
      ticketType <- TicketType.create(ticketTypeId, attractionId, ticketTypeName, description, unitPrice, createdAt).liftTo[F]
  ...
  override def addTicketEligibilityRule(
    ...
    for
      ...
      rule <- TicketEligibilityRule.create(ruleId, ticketTypeId, ruleType, ruleConfig, createdAt).liftTo[F]
