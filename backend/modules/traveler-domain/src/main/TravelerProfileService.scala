...
final class LiveTravelerProfileService[F[_]: MonadThrow](
  ...
  override def createTravelerProfile(
    ...
    for
      ...
      createdTravelerProfile = com.typesafe.travel.traveler.domain.newTravelerProfile(
        travelerId = travelerId,
        ...
        travelerType = TravelerType.deriveFromBirthDate(travelerBirthDate, currentDate),
        travelerPreferences = travelerPreferences,
  ...
  override def updateTravelerProfile(
    ...
    loadTravelerProfile(ownerUserId, travelerId)
      .flatMap { existingTravelerProfile =>
        currentLocalDate().flatMap { currentDate =>
        ensureTravelerDocumentIdentityIsAvailable(
          ...
          updatedTravelerType = TravelerType.deriveFromBirthDate(travelerBirthDate, currentDate),
