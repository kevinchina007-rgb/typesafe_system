...
final case class TicketEligibilityRule private (
...
object TicketEligibilityRule:
  def create(
    ...
    validateConfig(ruleType, ruleConfig).map(_ =>
  ...
  private def encodeRuleConfig(ruleConfig: TicketEligibilityRuleConfig): String =
    ruleConfig match
