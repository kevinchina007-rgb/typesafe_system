...
export function HotelDatePriceStrip({
  ...
  onNext,
  onDateSelect,
}: HotelDatePriceStripProps) {
  const dates = Array.from({ length: 7 }, (_, index) => addHotelDays(dateWindowStart, index))

  return (
    <section className="relative border border-sky-100 bg-gradient-to-r from-slate-950 via-sky-950 to-indigo-950 p-1 shadow-lg shadow-sky-200/40">
