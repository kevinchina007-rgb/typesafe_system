...
export function TourGroupOptionPicker({
  ...
  const title = useMemo(() => {
    ...
    return `${translate('tourGroups.createOption')} · ${localizeTourGroupItemType(planItem.itemType, currentLanguage)}`
  ...
  const content = (
    <div
      className={
        displayMode === 'modal'
          ? 'grid max-h-[90vh] w-full max-w-5xl gap-4 overflow-auto border border-slate-200 bg-white p-6 text-slate-950 shadow-2xl shadow-slate-950/20'
          : 'grid gap-4 border border-slate-200 bg-white p-4 text-slate-950 shadow-sm shadow-slate-200/50'
      }
      role="dialog"
      aria-modal={displayMode === 'modal' ? 'true' : undefined}
      aria-label={title}
    >
      <div className="flex items-start justify-between gap-4 text-lg font-bold text-slate-950">
        <div>
            ...
            {flightResults.map(flight => (
              <li key={flight.flightId}>
                <div>
                  ...
                  <p>{`${formatIsoDateTime(flight.departureTime, '-')} · ${mapBackendStatusToProductLabel(flight.status, currentLanguage)}`}</p>
                      ...
                      <button
                        ...
                        onClick={() =>
                          void addOption({
                            ...
                            description: `${flight.departureAirport} → ${flight.arrivalAirport} · ${formatIsoDateTime(flight.departureTime, '-')}`,
                            defaultQuantity: 1,
                          })
                        }
                      >
                          ...
                          onClick={() =>
                            quote
                              ? void addOption({
                                  ...
                                  description: `${trainFrom.toUpperCase()} → ${trainTo.toUpperCase()} · ${quote.amount} ${quote.currency}`,
                                  defaultQuantity: 1,
                                })
                              : undefined
                          }
                        >
                          {`${localizeTrainSeatClass(seat.seatClass, currentLanguage)} · ${quote ? `${quote.amount} ${quote.currency}` : translate('trains.routePriceUnavailable')}`}
                        </button>
                      )
                    })}
                  </div>
