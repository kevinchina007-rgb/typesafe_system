...
import { filterAttractionSessionsForUseDate, formatAttractionRules, renderAttractionTravelerOptionLabel } from '../attractionBookingModel'
                        ...
                        <select name="sessionId" defaultValue="" required disabled={isBusy}>
                          ...
                          {filterAttractionSessionsForUseDate(ticketType.sessions, useDateDraft).map(session => (
                            <option key={session.sessionId} value={session.sessionId}>
                              {`${session.sessionName} 场 ${session.startsAt.slice(11, 16)}-${session.endsAt.slice(11, 16)}`}
                            </option>
                          ))}
                        </select>
                      </label>
