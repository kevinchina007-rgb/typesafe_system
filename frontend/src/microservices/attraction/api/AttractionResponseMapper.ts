...
function mapAttractionTicketTypeRule(rule: BackendAttractionTicketRule) {
  ...
  const summary = (() => {
    if (ruleType === 'AgeLessThan' && ageValue !== null) {
      return `游客当日年龄必须小于 ${ageValue} 岁`
    }
    if (ruleType === 'AgeBetween' && minAge !== null && maxAge !== null) {
      return `游客当日年龄必须在 ${minAge}-${maxAge} 岁之间`
    }
    if (ruleType === 'AgeAtLeast' && (minAge !== null || ageValue !== null)) {
      return `游客当日年龄必须至少 ${minAge ?? ageValue} 岁`
    }
...
function mapAttractionTicketSession(session: BackendAttractionTicketSession) {
  return {
...
function mapAttractionTicketType(ticketType: BackendAttractionTicketType, useDate?: string) {
...
export function mapAttractionResponseFromBackend(response: BackendAttractionResponse, useDate?: string): AttractionResponse {
  return {
    ...
    status: response.attractionStatus,
    ticketTypes: response.ticketTypes.map(ticketType => mapAttractionTicketType(ticketType, useDate)),
  }
}

export function mapAttractionListResponseFromBackend(response: BackendAttractionListResponse, useDate?: string): AttractionListPlannerResponse {
  return {
    attractions: response.attractions.map(attraction => mapAttractionResponseFromBackend(attraction, useDate)),
  }
}
